...
class BackOfficeActionsLoggerListener implements EventSubscriberInterface
{
    ...
    public function __construct( SiteAccess $siteAccess, TokenStorage $tokenStorage, Repository $repository, $logDir,
                                 RequestStack $requestStack, TagsService $tagsService, $adminLoggerEnabled )
    {
    ...
    public function onAPISignal( SignalEvent $event )
    {
        ...
        if( !$this->adminLoggerEnabled || stripos( $this->siteAccess->name, "admin" ) === false )
        {
            return;
        }
    ...
    private function logIfCopyContentSignal( $signal )
    {
        if( $signal instanceof Signal\ContentService\CopyContentSignal )
        {
            ...
            try
            {
                ...
                $this->logger->addNotice( "  - destination parent content name : " . $dstParentLocation->getContent()->getName() );
            }
            catch( \Exception $exception )
            {
    ...
    private function logIfMoveSubtreeSignal( $signal )
    {
        if( $signal instanceof Signal\LocationService\MoveSubtreeSignal )
        {
            ...
            try
            {
                $oldParentLocation = $this->repository->getLocationService()->loadLocation( $signal->oldParentLocationId );
                $this->logger->addNotice( "  - old parent content name : " . $oldParentLocation->getContent()->getName() );
            }
            catch( \Exception $exception )
            {
            ...
            try
            {
                $newParentLocation = $this->repository->getLocationService()->loadLocation( $signal->newParentLocationId );
                $this->logger->addNotice( "  - new parent content name : " . $newParentLocation->getContent()->getName() );
            }
            catch( \Exception $exception )
            {
    ...
    private function logIfCopySubtreeSignal( $signal )
    {
        if( $signal instanceof Signal\LocationService\CopySubtreeSignal )
        {
            ...
            try
            {
                $newParentLocation = $this->repository->getLocationService()->loadLocation( $signal->targetParentLocationId );
                $this->logger->addNotice( "  - copy's parent content name : " . $newParentLocation->getContent()->getName() );
            }
            catch( \Exception $exception )
            {
    ...
    private function logUserInformations(): void
    {
        ...
        $this->logger->addNotice( sprintf( "  - user name : %s [contentId=%s]",
                                           $user->getUsername(),
